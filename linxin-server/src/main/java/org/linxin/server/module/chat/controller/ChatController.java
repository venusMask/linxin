package org.linxin.server.module.chat.controller;
...
public class ChatController {
    ...
    @GetMapping("/conversations/ai")
    @Operation(summary = "获取或创建与系统AI的会话")
    public Result<ConversationVO> getOrCreateAIConversation(
            @RequestAttribute("userId") Long userId) {
        return Result.success(chatService.getOrCreateAIConversation(userId));
    }
    ...
    public Result<IPage<MessageVO>> getMessageList(
            ...
            @RequestParam(defaultValue = "1") Integer pageNum,
            @RequestParam(defaultValue = "20") Integer pageSize) {
        // 安全校验：确保 conversationId 属于当前登录用户，防止 IDOR 越权
        if (!chatService.isConversationOwner(userId, conversationId)) {
