...
class _AIChatPageState extends State<AIChatPage> {
  ...
  Future<void> _initAIChat() async {
    try {
      // 通过专用接口直接获取或创建 AI 会话，不再需要预先知道 AI ID
      final response = await _httpService.get('/chat/conversations/ai');
  ...
  Future<void> _sendMessage() async {
    ...
    // 增加 30 秒超时保护
    _aiThinkingTimer?.cancel();
    _aiThinkingTimer = Timer(const Duration(seconds: 30), () {
      if (mounted && _isAiThinking) {
        setState(() {
          _isAiThinking = false;
        });
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('AI 无响应，请稍后再试')),
        );
      }
    });
